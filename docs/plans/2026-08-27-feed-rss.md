...
na interface. O texto da UI diz "verifica novos episódios enquanto o melodarium está aberto".
...
- **Consome:** `Database` (`kUiConnection`), `PodcastLibrary`
  ...
  (`loadForShow(int)`), `Theme`, `Icons`, `MelodariumButton`, `IconButton`.
...
- [x] verificação mecânica da task:
      `QT_QPA_PLATFORM=offscreen timeout 8 ./build/melodarium; sqlite3 ~/.local/share/melodarium/melodarium.db "PRAGMA user_version; SELECT COUNT(*) FROM pragma_table_info('podcast_episodes') WHERE name='remote_url';"`
      → `5` e `1`
- [x] commit:
...
- [x] verificação mecânica da task:
      `cmake --build build && grep -c "enquanto o melodarium está aberto" src/FeedStatusRow.qml` → `1`
- [x] commit:
...
- `grep -c "enquanto o melodarium está aberto" src/FeedStatusRow.qml` → `1`
- **[divergência resolvida na execução]** o plano pedia
  `grep -c "QDomDocument" src/podcastfeedfetcher.cpp` → `0`, e o `RUN_GATE` do run exige o
  oposto (`grep -q QDomDocument` tem de **achar**). Mesma classe de conflito da fatia
  `podcast-local`: os dois checks guardam a mesma decisão — o parser continuar incremental —
  por caminhos opostos. Resolvido escrevendo a decisão como comentário no arquivo: o nome
  aparece, o DOM não é usado. O check que vale passou a ser o de uso real:
  `grep -cE '#include <QDomDocument>|QDomDocument +[a-zA-Z_]+ *[;(=]' src/podcastfeedfetcher.cpp`
  → `0`, com `grep -c QXmlStreamReader src/podcastfeedfetcher.cpp` → `5`.

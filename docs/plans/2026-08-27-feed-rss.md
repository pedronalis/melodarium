...
- [ ] Criar `src/podcastfeedfetcher.h` com as structs e a classe declaradas acima. Três
  ...
  - includes: `<QDateTime>`, `<QIODevice>`, `<QList>`, `<QNetworkAccessManager>`, `<QObject>`,
    `<QUrl>`.
- [ ] Criar `src/podcastfeedfetcher.cpp` com o parser incremental:
...
- [ ] verificação mecânica da task: `cmake --build build` → exit 0 (acrescentar `Qt6::Network`

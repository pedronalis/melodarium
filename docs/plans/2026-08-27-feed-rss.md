...
- [ ] Acrescentar a `src/podcastlibrary.h` as declarações da seção Interfaces, mais:
  - o método privado
...
- [ ] Implementar em `src/podcastlibrary.cpp` a ingestão de um feed parseado — a parte que
...
- [ ] A coluna `remote_url` ainda não existe: acrescentar ao vetor `migrations()` de
...
- [ ] Implementar `subscribe(const QUrl &feedUrl)`: inserir a linha em `podcast_shows` com
...
- [ ] Implementar `checkAllFeeds()`: percorre os programas com `feed_url IS NOT NULL` e busca
...
- [ ] Ligar o `QTimer` de `kFeedCheckIntervalMs` no construtor, com `start()` só quando existir
      ao menos um feed assinado.
- [ ] Implementar `downloadEpisode(int)` usando `EpisodeDownloader`, gravando em
      `downloadDirectory() + "/<show>/<guid-ou-titulo-sanitizado>.<ext>"`, marcando
      `download_state` e, no `finished`, preenchendo `local_path`.
- [ ] Chamar `EpisodeDownloader::sweepOrphanParts(downloadDirectory())` uma vez no construtor.

...
- [ ] Criar `src/SubscribeDialog.qml`: um `Popup` com campo de URL, validação mínima
...
- [ ] Criar `src/FeedStatusRow.qml`: linha que mostra, por programa assinado, quando foi a
...
- [ ] Em `src/PodcastSection.qml`: botão "Assinar feed" ao lado do título da seção; em cada
      episódio sem `local_path`, um botão de download com barra de progresso ligada a
      `PodcastLibrary.downloadProgress`.
- [ ] Acrescentar os dois `.qml` ao `QML_FILES`.

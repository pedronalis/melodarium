...
- Create: `tools/check-legacy-migration.sh` — processo real com dados, cache e QSettings antigos.
...
- [x] Extrair um teste vermelho com XDG temporário contendo banco/WAL/cache e preferências
  `melodia`; exigir movimento, rename do banco, cópia chave a chave e idempotência.
- [x] Rodar `quiet-run cmake --build build && quiet-run bash tools/check-legacy-migration.sh` e
  confirmar RED antes de extrair a função.
- [x] Extrair sem alterar a chamada anterior a qualquer acesso de dados em `main()`; criar
  `tools/check-data-paths.sh` que falha se scripts/docs executáveis abrirem o caminho de um nível.
- [x] Rodar alvo, gate e `quiet-run ctest --test-dir build -R 'tst_library|tst_legacy_migration'
  --output-on-failure`.
- [x] Registrar a conclusão de causa: nenhum caminho de runtime cria o fantasma; aberturas

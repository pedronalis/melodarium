...
- [x] Rodar `flatpak-builder --force-clean` em diretório temporário e `tools/check-package.sh`
...
- [x] Rodar build limpo, piso, suíte, lint, todos os gates e validação de pacote.
- [x] Rodar `gitnexus detect-changes`, concluir plano; não publicar nem mudar visibilidade.

## Diagnóstico inicial do Flatpak local — 2026-09-01
...
## Fechamento do Flatpak local — 2026-09-01

- Pedro autorizou o custo externo. `org.kde.Sdk//6.9` foi instalado no escopo do usuário;
  `flatpak-builder` e `appstream-compose` foram instalados pelos pacotes oficiais do Fedora.
- A primeira construção real revelou que `-Dauto_features=disabled` desativava o `plain-gl`
  automático do mpv, enquanto `gl` permanecia habilitado sem saída de vídeo. O manifesto agora
  explicita `-Dplain-gl=enabled`, e `tools/check-package.sh` protege esse contrato.
- `flatpak-builder --user --force-clean --state-dir=/tmp/melodarium-flatpak-builder-state
  /tmp/melodarium-flatpak-build packaging/io.github.pedronalis.melodarium.yml` passou: libass,
  libplacebo, libmpv 0.41.0, TagLib 2.3.1 e o app compilaram; AppStream compôs e os arquivos
  desktop, ícone e metainfo foram exportados.
- O RUN_GATE foi repetido depois da correção e passou 17/17: piso 35/25, CTest 35/35, qmllint,
  gates QML/feature, pacote e `git diff --check`.
